...
public class Day1 {
    ...
    public static int part1(String instructions) {
        ...
        var elfSums = elfLevel.map(Day1::getElfSum);
    ...
    public static int part2(String instructions) {
        var elfLevel = Arrays.stream(instructions.split(ELF_DELIMITER));
        var elfSumsSorted = elfLevel.map(Day1::getElfSum).sorted(Comparator.reverseOrder());
        return elfSumsSorted.limit(3).mapToInt(Integer::valueOf).sum();
    }

    public static void main(String[] args){
        try {
            InputStream i = Day1.class.getClassLoader().getResourceAsStream("2022/day1.txt");
